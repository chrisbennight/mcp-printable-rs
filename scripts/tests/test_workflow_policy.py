...
class WorkflowPolicyTests(unittest.TestCase):
    ...
    def test_unsafe_workflow_mutations(self):
        cases = [
            ...
            ("release.yml", lambda w: w["jobs"]["gpu"].update({"permissions": {"packages": "write"}})),
            ("release.yml", lambda w: w["jobs"]["publish"].update({"needs": ["build"]})),
            ("release.yml", lambda w: w["jobs"]["build"].update({"runs-on": "self-hosted"})),
            ("release.yml", lambda w: w["jobs"]["publish"].update({"runs-on": "self-hosted"})),
